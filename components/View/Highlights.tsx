      ...
      <div className='grid grid-cols-1 justify-center sm:grid-cols-2 lg:grid-cols-3 gap-2 my-14'>
        {cloudinaryList?.map(({ public_id, _key, format, version }, indx) => (
          <div
            ...
            onClick={() =>
              openImageModal(
                `https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/h_720,c_scale/v${version}/${public_id}.${format}`
              )
            }
          >
            <Image
              id={_key}
              src={`https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/w_720,c_scale/v${version}/${public_id}.${format}`}
              alt={public_id}
              fill
